    ...
    <AppShell active="gerencial">
      <header className="mb-8">
        ...
        <p className="mt-1 text-on-surface-variant text-sm sm:text-base">
          Competência atual · dados de demonstração · repasse ilustrativo
        </p>
      </header>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
        <div className="rounded-xl bg-surface border border-outline-variant/40 p-6 shadow-ambient flex flex-col items-center text-center">
          ...
          <span className={`mt-2 rounded-full px-3 py-1 text-xs font-semibold ${mediaClass.estilo}`}>
            {mediaClass.label}
          </span>
        </div>
        <div className="rounded-xl bg-surface border border-outline-variant/40 p-6 shadow-ambient flex flex-col items-center text-center relative overflow-hidden">
          ...
          <p className="relative text-xs font-semibold uppercase tracking-wider text-on-surface-variant mb-3">
            Repasse mensal (sim.)
          </p>
          <p className="relative text-3xl font-bold text-primary-container">
            {formatarReais(totalMensal)}
          </p>
          <p className="relative text-sm text-on-surface-variant mt-1">
            {MUNICIPIO_MOCK.equipes.length} equipes
          </p>
        </div>
          ...
          <table className="w-full text-left min-w-[640px]">
            <thead>
              <tr className="bg-surface-container-low border-b border-outline-variant/40">
                {['Equipe', 'Tipo', 'Nota ISF', 'Classificação', 'Repasse (sim.)'].map((h, i) => (
                  <th
                    key={h}
                    className={`py-3.5 px-4 text-xs font-semibold uppercase tracking-wider text-on-surface-variant ${i === 4 ? 'text-right' : ''}`}
                  >
                    {h}
                  </th>
                ))}
            ...
            <tbody className="divide-y divide-outline-variant/20">
              {porEquipe.map((r) => {
                ...
                return (
                  <tr key={r.equipe.id} className="hover:bg-surface-container-low/80">
                    <td className="py-3.5 px-4 font-medium text-on-surface">{r.equipe.nome}</td>
                    <td className="py-3.5 px-4 text-on-surface-variant">
                      <Link href={`/paineis/${tipo.id}`} className="hover:text-primary underline-offset-2 hover:underline">
                        {tipo.nome}
                      </Link>
                    </td>
                    <td className="py-3.5 px-4 font-mono">{r.nota.toFixed(1).replace('.', ',')}</td>
                    <td className="py-3.5 px-4">
                      <span className={`rounded-full px-2.5 py-1 text-xs font-semibold ${cls.estilo}`}>
                        {cls.label}
                      </span>
                    </td>
      ...
      <p className="text-sm text-on-surface-variant mb-8">
        Valores ilustrativos. Para projeções detalhadas, use o{' '}
        <Link href="/simulador" className="text-primary font-medium underline-offset-2 hover:underline">
      ...
      <div className="rounded-xl bg-surface border border-outline-variant/40 p-6 shadow-ambient">
        ...
        <InsightsPanel
          indicadores={[]}
          equipes={porEquipe.map((r) => ({
            nome: r.equipe.nome,
            nota: r.nota,
            classificacao: classificacaoDaNota(r.nota).label,
          }))}
        />
      </div>
    </AppShell>
